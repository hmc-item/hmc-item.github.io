...
printf '%s' '{"id":"ths_smoke","section_key":"smoke","subject":"기계보전","section_title":"PLC","content":{"t":"℃±㎛² 무손실 테스트"},"status":"draft"}' > "$TMP"
# INSERT
curl -s -o /dev/null -w "INSERT %{http_code}\n" -X POST "$URL/theory_sections" \
  -H "$H_KEY" -H "$H_AUTH" -H "$H_JSON" --data-binary @"$TMP"
# SELECT 무손실 확인
curl -s "$URL/theory_sections?section_key=eq.smoke&select=content" -H "$H_KEY" -H "$H_AUTH"; echo ""
# DELETE 정리
